...
// is covered separately in e2e-tests/journeys/creation-prompts.e2e.ts.
describe("auto-attach template", () => {
  before(async () => {
    await browser.reloadObsidian({ vault: "./e2e-tests/fixtures/e2e-daily-template", plugins: ["journals"] });
  });
