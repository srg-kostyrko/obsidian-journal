...
describe("views examples", () => {
  describe("sidebar view (#116)", () => {
    before(async () => {
      ...
      await reloadObsidianOn(
        TODAY,
        { vault: "./e2e-tests/fixtures/e2e-docs-views-sidebar", plugins: ["journals"] },
        "01:58:00",
      );
    });
  ...
  describe("week numbers after weekdays (#148)", () => {
    before(async () => {
      await reloadObsidianOn(TODAY, {
        vault: "./e2e-tests/fixtures/e2e-docs-views-week-numbers",
        plugins: ["journals"],
      });
    });
  ...
  describe("URI: open next week's note in a new tab", () => {
    before(async () => {
      await reloadObsidianOn(URI_DAY, { vault: "./e2e-tests/fixtures/e2e-uri", plugins: ["journals"] });
    });
