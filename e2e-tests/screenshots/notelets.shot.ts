...
describe("notelets examples", () => {
  before(async () => {
    await reloadObsidianOn(TODAY, { vault: "./e2e-tests/fixtures/e2e-notelets", plugins: ["journals"] });
  });
...
describe("questions examples", () => {
  before(async () => {
    await browser.reloadObsidian({ vault: "./e2e-tests/fixtures/e2e-prompts", plugins: ["journals"] });
  });
