...
describe("navigation blocks examples", () => {
  before(async () => {
    await reloadObsidianOn(TODAY, { vault: "./e2e-tests/fixtures/e2e-docs-navigation-blocks", plugins: ["journals"] });
  });
...
describe("navigation blocks examples — several segments on one line", () => {
  before(async () => {
    await reloadObsidianOn(TODAY, {
      vault: "./e2e-tests/fixtures/e2e-docs-navigation-blocks-one-line",
      plugins: ["journals"],
    });
  });
