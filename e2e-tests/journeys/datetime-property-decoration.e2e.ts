...
//
// A dedicated fixture (e2e-tests/fixtures/e2e-datetime-deco), not the shared e2e-journeys one: its
...
describe("date-and-time property decoration conditions", () => {
  before(async () => {
    await browser.reloadObsidian({ vault: "./e2e-tests/fixtures/e2e-datetime-deco", plugins: ["journals"] });
