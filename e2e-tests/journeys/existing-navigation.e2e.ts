...
// The existing-navigation toolbar item mounts in a real Obsidian view leaf. Clicking its
...
const RIBBON_OPEN = '[aria-label="Open Existing Nav"]';
...
describe("existing-navigation toolbar", () => {
  before(async () => {
    await browser.reloadObsidian({ vault: "./e2e-tests/fixtures/e2e-existing-nav", plugins: ["journals"] });
  ...
  it("opens the nearest earlier existing note when the previous arrow is clicked", async () => {
    ...
    await $(MONTH_VIEW).waitForExist({ timeoutMsg: "existing-nav view did not render after the ribbon click" });
    await $(PREVIOUS_ARROW).waitForExist({ timeoutMsg: "existing-navigation previous arrow did not render" });
