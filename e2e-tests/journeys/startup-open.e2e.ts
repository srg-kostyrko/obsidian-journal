...
describe("open on startup", () => {
  before(async () => {
    await browser.reloadObsidian({ vault: "./e2e-tests/fixtures/e2e-startup", plugins: ["journals"] });
  });
...
describe("open on startup with a weekday override", () => {
  before(async () => {
    await browser.reloadObsidian({ vault: "./e2e-tests/fixtures/e2e-startup-weekday", plugins: ["journals"] });
  });
...
describe("open on startup, pinned", () => {
  before(async () => {
    await browser.reloadObsidian({ vault: "./e2e-tests/fixtures/e2e-startup-pinned", plugins: ["journals"] });
  });
