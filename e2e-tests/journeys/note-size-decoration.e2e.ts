...
//
// A dedicated fixture (e2e-tests/fixtures/e2e-note-size), not the shared e2e-journeys one: its
...
describe("note size decoration condition", () => {
  before(async () => {
    await browser.reloadObsidian({ vault: "./e2e-tests/fixtures/e2e-note-size", plugins: ["journals"] });
  });
