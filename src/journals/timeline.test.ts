...
describe("TimelineService", () => {
  describe("contains", () => {
    describe("daily journal with an open-ended timeline starting 2024-01-01", () => {
      ...
      it("returns true for anchor at or after start when end.kind is never", () => {
        const timeline = harness.resolve(TimelineService);
    ...
    it("returns false for anchor after end.date", async () => {
      ...
      const timeline = resolve(TimelineService);
      expect(timeline.contains("daily", "2024-07-01" as AnchorString)).toBe(false);
    });

    it("treats an unset end date as no upper bound", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
    ...
    it("treats a repeats bound as no bound when the timeline has no start", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
    ...
    it("includes the period containing the start date when the start falls after that period's anchor", async () => {
  ...
  describe("startOf", () => {
    it("returns the start CalendarDate for a known journal", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
    ...
    it("returns None when the timeline has no start", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
  ...
  describe("endOf", () => {
    it("returns None for end.kind === never", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
    ...
    it("returns Some(date) for end.kind === date", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
    ...
    it("returns None for a repeats bound with no timeline start", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
    ...
    it("returns None when the end date is unset", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
    ...
    it("returns the computed end date for end.kind === repeats", async () => {
  ...
  describe("boundsOf", () => {
    it("leaves both sides open for an unknown journal", async () => {
      const { resolve } = await testContainer({ modules: [journalsCoreModule], data: { journals: {} } });
      const bounds = resolve(TimelineService).boundsOf("missing");
      expect(bounds.start.isNone() && bounds.end.isNone()).toBe(true);
    });

    it("leaves the lower side open when the timeline has no start", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
      ...
      expect(resolve(TimelineService).boundsOf("daily").start.isNone()).toBe(true);
    });
    ...
    describe("daily journal with an open-ended timeline starting 2024-01-01", () => {
      ...
      it("leaves the upper side open when the timeline never ends", () => {
        expect(harness.resolve(TimelineService).boundsOf("daily").end.isNone()).toBe(true);
      });

      it("bounds a daily journal at its configured start date", () => {
        const bounds = harness.resolve(TimelineService).boundsOf("daily");
        expect(bounds.start.match({ some: (d) => d.toAnchor(), none: () => null })).toBe("2024-01-01");
      });
    });

    it("widens the lower bound to the start of the week a mid-week start falls in", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
      ...
      const bounds = resolve(TimelineService).boundsOf("weekly");
      expect(bounds.start.match({ some: (d) => d.toAnchor(), none: () => null })).toBe("2026-06-01");
    });

    describe("weekly journal whose timeline ends 2026-06-03", () => {
      let harness: TestHarness;
      ...
      it("extends the upper bound to the end of the week the timeline end falls in", () => {
        const bounds = harness.resolve(TimelineService).boundsOf("weekly");
    ...
    it("keeps the upper bound at the end date when it already closes a period", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
      ...
      const bounds = resolve(TimelineService).boundsOf("daily");
      expect(bounds.end.match({ some: (d) => d.toAnchor(), none: () => null })).toBe("2026-06-03");
    });

    it("bounds a repeats end at the last repeat", async () => {
      const { resolve } = await testContainer({
        modules: [journalsCoreModule],
        data: {
          journals: {
    ...
    describe("custom daily-cycle journal whose timeline ends 2026-06-03", () => {
      let harness: TestHarness;
          ...
          data: {
            journals: {
              custom: customJournal("custom", "day", 7, "2026-06-01", {
                timeline: {
                  start: "2026-06-01" as AnchorString,
                  end: { kind: "date", date: "2026-06-03" as AnchorString },
                },
              }),
            },
          },
      ...
      it("extends the upper bound to the end of the interval a custom-interval timeline end falls in", () => {
        const bounds = harness.resolve(TimelineService).boundsOf("custom");
        expect(bounds.end.match({ some: (d) => d.toAnchor(), none: () => null })).toBe("2026-06-07");
      });
    ...
    it("rejects the interval past an off-anchor repeats start consistently with contains", async () => {
    ...
    it("agrees with contains for a custom journal with an extended interval", async () => {
