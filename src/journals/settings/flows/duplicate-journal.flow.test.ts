...
import { DuplicateJournalFlow } from "./duplicate-journal.flow";
...
async function build(initial: Record<string, JournalConfig> = {}) {
  ...
  container.register(DuplicateJournalFlow).useClass(DuplicateJournalFlow);
  return {
    storage,
    modals,
...
describe("DuplicateJournalFlow", () => {
  ...
  it("opens the modal with a free suggested name derived from the source", async () => {
    const { flows, modals } = await build({ daily: daily() });
    void flows.invoke(DuplicateJournalFlow, { journalName: "daily" });
  ...
  it("returns UserAborted('duplicate-journal-modal') when the modal is cancelled", async () => {
    const { flows, modals } = await build({ daily: daily() });
    const promise = flows.invoke(DuplicateJournalFlow, { journalName: "daily" });
    modals.lastOpen().cancel();
    const result = await promise;
    expect(result.kind === "err" && result.error).toBeInstanceOf(UserAborted);
    expect(result.kind === "err" && (result.error as UserAborted).source).toBe("duplicate-journal-modal");
  });
  ...
  it("fails without opening a modal when the source journal is gone", async () => {
    const { flows, modals } = await build();
    const result = await flows.invoke(DuplicateJournalFlow, { journalName: "daily" });
    expect(result.kind === "err" && (result.error as JournalLifecycleFlowError).cause).toBeInstanceOf(
      UnknownJournalError,
    );
