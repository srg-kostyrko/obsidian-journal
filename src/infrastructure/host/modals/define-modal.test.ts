...
describe("defineModal", () => {
  it("infers TProps and TResult from explicit type arguments", () => {
    const definition = defineModal<{ name: string }, number>({
      component: Stub,
      title: ({ name }) => name,
    });
  ...
  it("defaults TProps and TResult to void when omitted", () => {
    const definition = defineModal({ component: Stub, title: () => "x" });
    expectTypeOf(definition).toEqualTypeOf<ModalDefinition<void, void>>();
  });

  it("normalizes a numeric width into a (props) => number function", () => {
    const definition = defineModal<{ name: string }, void>({
      component: Stub,
      title: ({ name }) => name,
      width: 520,
    });
