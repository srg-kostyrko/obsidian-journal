...
- A journal that splits its date across folders and the note name, where one part also carries the day of the week, now reads the date back out of the path — including `YYYY/MM-MMM/DD-ddd`, the daily layout Periodic Notes uses. A weekday anywhere in the date made the plugin read each part on its own, and no part alone names a day, so the date was never recovered: a daily note you made yourself in that layout was never attached to its journal, **Maintenance** could not fall back on the path to repair a connected note whose stored date went missing or was damaged, and the journal's settings warned that the date was too coarse to tell the days apart. A path whose weekday does not match its date is still not read as that date.
