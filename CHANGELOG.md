...
- A journal whose note names cannot be read back now says so for the right reason. Where every period got its own name and the plugin still could not recover a date from it — a localized date format such as `LL`, or a timestamp, both of which are written correctly and match nothing on the way back — the settings page said the date variable was not precise enough to tell the journal's periods apart, which was not true of a name that changes every day. The warning now tells those two cases apart by checking whether two neighbouring periods really do share a note name, and the new one names the cause and what to use instead. A name that genuinely repeats across periods still gets the old wording.
