...
- [ ] - Open a vault with a v2 `data.json` → loads without crash (migration, §10).
...
Cfg. After each change, create a _new_ entry to observe the effect (existing
notes should be untouched).
...
- [ ] **Template** — + add `templates/daily template.md` → new note's body is the
      template content.
- [ ] **Multiple templates** listed → all applied in order.
...
- [ ] **Frontmatter date field** renamed → new note's frontmatter uses the new
      key.
- [ ] **Custom-interval start/end date fields** (+ on a custom journal) → both
      written with configured key names.
...
- [ ] **Delete → delete notes** → notes moved to **trash** (recoverable), not
      permanently erased.
- [ ] - Create a second journal with the **same name** → "colliding journals"
    warning shows in the settings dashboard.
...
- [ ] **Two numbering sources** → each writes its own frontmatter key
      independently.
...
- [ ] month-calendar **hideWeekends** → Sat/Sun columns hidden.
...
- [ ] - Create-journal flow with a shelf chosen → new journal lands on that shelf.
...
Setup: Day journal "Deco" with notes on several dates; open a calendar view
showing it. Add decorations via Settings → Journals → Deco → Decorations. For each
...
- [ ] **offset = +1** → the cell one position after the journal's anchor is
      decorated.
- [ ] **offset = −1** → the cell one position before the anchor is decorated.
...
- [ ] Dashboard renders blocks in order: colliding-journals, calendar-week,
      calendar-appearance, shelves, journals, commands, views, startup, logging.
...
Setup: **back up `data.json` first.** Use real legacy snapshots, not a clean
vault. Reload after each to let async note migration run.
