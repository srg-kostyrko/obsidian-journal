...
- [x] **59. Date picker opens on today, not the displayed month, with no pre-selection** — fixed: `ButtonItem` passes the period containing `context.refDate` as `selected` into `datePickerModal` (both the level path and the pinned-journal path), so the picker opens where the calendar is and highlights it. Test watched red first. — v2 passed `selectedDate: refDate` (`CalendarView.vue:103`); v3 opens with `{ picking }` only (`ButtonItem.vue:75`, `DatePickerModal.vue:58-64`).
- [x] **60. Month grid no longer follows an opened custom-interval note** — fixed: the month/week-calendar blocks' `useFollowActiveDate` scope widened from `scope.fixed` to `scope.all`, so opening a custom-interval note recenters the panel (v2 behavior); the cell highlight stays fixed-journal-scoped. Test watched red first. — v2's watcher recentered on any journal type (`CalendarView.vue:128-162`); v3's month block follows `scope.fixed` only (`MonthCalendarBlock.vue:29`).
- [x] **61. Cell label format drift** — fixed: `defaultFormatPattern` back to v2's `[W]w` (unpadded week) and `MMMM` (full month name in the in-grid heading); the toolbar month badge already used `MMMM`. — week cells `W01` vs v2 `W1`, in-grid month heading `Jan` vs `January` (`src/notes-calendar/cell-format.ts` vs `src/_old-code/constants.ts:20-26`).
- [~] **62. Home block: `separator: ""` / `scale: 0` no longer coerce to defaults** — kept as a v3 extension: an explicit empty separator (or tiny scale) is now expressible where v2 silently coerced falsy values. Reopen if the `scale: 0` footgun bites. (`src/_old-code/code-blocks/home/home-processor.ts:44-45` `||` vs `v.optional` in `home-config.ts:9-10`).
- [x] **63. Timeline month mode: adjacent-month days no longer clickable** — fixed: `TimelineMonth` passes `outside-dates="active"` (v2 parity); quarter/calendar modes still blank them. — v3 passes `outside-dates="inactive"` (`src/code-blocks/timeline/ui/TimelineMonth.vue:19`); v2 left them actionable.
- [x] **64. `end_date` written even when equal to the period start** — kept: v3 writes the explicit end; the v2 suppression was a niche quirk with no user-facing value. No change. (v2 equality guard `journal.ts:631-640`; v3 `frontmatter.ts:123-133`). Niche custom-interval delta.
- [x] **65. Invalid `end_date` frontmatter: v2 hid the whole note, v3 indexes it without the end span** — kept: v3's tolerance is strictly better (the note stays reachable). No change. (`src/_old-code/journals/journals-index.ts:97` vs `src/journals/frontmatter.ts:45-50`). v3 is more tolerant — likely fine, confirm and close.
